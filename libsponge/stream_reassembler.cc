...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    for (size_t i = 0; i < data.size(); ++i) {
        if (_unassem_buffer.count(i + index))
    ...
    while (i < data.size()) {
        if (i + index == _unassem_base) {            // 当前字符就是期望字符
            if (_output.remaining_capacity() > 0) {  // 如果_output的buffer未满，则考虑将该字符输入到ByteStream中
                ...
                _unassem_base += 1;
            } else if (_unassem_buffer.size() < _capacity)
                _unassem_buffer[i + index] = data[i];  // _output的buffer已满时，放在_unassem_buffer中

        } else if (i + index > _unassem_base &&
                   _unassem_buffer.size() < _capacity) {  // 如果是尚未重组的字符，则将其输入到unassembled buffer中
            _unassem_buffer[i + index] = data[i];
        }
